...
public class JiraApiRestSteps {
    ...
    public void the_has_an_authorization_in_jira() throws IOException, URISyntaxException {
        jiraCookie = PayloadUtil.getJirasessionCookie();
    }
    ...
    public void the_user_creates_issues_with_sprintID(String summary, String description, String issueType) {
        ...
        issueType = PayloadUtil.generateIssueType();

        for (int i = 0; i < 30; i++) {
            response = given().contentType(ContentType.JSON)
                    .accept(ContentType.JSON).header("Cookie", jiraCookie)
                    .body(PayloadUtil.getIssuePayloadWithSprintId(summary, description, issueType, PayloadUtil.sprintIDs.get(0)))
